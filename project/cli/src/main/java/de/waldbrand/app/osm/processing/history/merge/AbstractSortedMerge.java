...
public class AbstractSortedMerge extends AbstractMerge
{
			...
			OsmIterator iterator)
	{
		...
		item.currentVersion = item.currentEntity.getMetadata().getVersion();
		return item;
	}
	...
	protected boolean advanceNodeItem(Input<OsmNode> item, boolean putBack)
	{
		...
		if (container.getType() == EntityType.Node) {
			...
			item.currentVersion = entity.getMetadata().getVersion();
			if (putBack) {
				nodeItems.add(item);
			}
			return true;
			...
			newItem.currentVersion = entity.getMetadata().getVersion();
