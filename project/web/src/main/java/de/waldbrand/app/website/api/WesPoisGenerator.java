...
public class WesPoisGenerator implements ApiEndpoint
{

	private HttpServletRequest request;

	public WesPoisGenerator(HttpServletRequest request)
	{
			...
			Map<String, String[]> parameters) throws IOException
	{
		...
		if (useForst) {
			for (WesType type : WesType.values()) {
				...
				Icon icon = IconMapping.get(type);
				markers.addWes("forst-" + type.getId(), icon.getName(),
						filterLbf(only(data.getWesPois(), type.getId()),
								envelope));
			}
		}
